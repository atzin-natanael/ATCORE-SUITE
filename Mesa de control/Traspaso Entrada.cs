...
{
    public partial class Traspaso_Entrada : Form
    {
        List<List<string>> ArticulosTraspasoE = new List<List<string>>();
        string Docto_In_Id = "";
        public Traspaso_Entrada()
        {
            InitializeComponent();
        }
            ...
            if (conecta == 1)
            {
                ...
                else if (final == 3)
                {
                    ...
                    for (int i = 0; i < ArticulosTraspasoE.Count; ++i)
                    {
                        ...
                        {
                            mensaje.Texto.Text += ArticulosTraspasoE[i][1] + " --- " + ArticulosTraspasoE[i][3] + " de " + ExArt + " disponibles" + "\n";
                        }
                    }
                    mensaje.Texto.SelectionAlignment = HorizontalAlignment.Center;
                    mensaje.ShowDialog();
        ...
        private void CargarGrid()
        {
            if (TxtFolio.Text != string.Empty)
            {
                ...
                string query = "SELECT DOCTOS_IN_ID, REPETIDO FROM DOCTOS_IN WHERE FOLIO LIKE '" + coriba + "%' AND FOLIO = '" + TxtFolio.Text + "' AND TIPO_DOCTO = 'T';";
                ...
                foreach (var articulo in ListaArticulos)
                {
                    ...
                    ArticulosTraspasoE.Add(ArticuloC);
